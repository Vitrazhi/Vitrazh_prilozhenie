...
function loadState() {
  try {
    ...
    if (!raw) return { goals: [], completedGoals: [] };
    const data = JSON.parse(raw);
    if (!data.goals || !Array.isArray(data.goals)) return { goals: [], completedGoals: [] };
    if (!data.completedGoals || !Array.isArray(data.completedGoals)) data.completedGoals = [];
    return data;
  } catch {
...
function saveState(state) {
  localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({ goals: state.goals, completedGoals: state.completedGoals || [] }),
  );
}
...
function ensureCompletedFields(goal) {
  ensureGoalFields(goal);
  if (!goal.completedAt) goal.completedAt = Date.now();
}
...
function refreshListView() {
  ...
  if (!state.completedGoals.length) {
    cempty.classList.remove("hidden");
  } else {
    cempty.classList.add("hidden");
    for (const g of state.completedGoals) {
      ensureCompletedFields(g);
      ensureGoalShards(g);
      const li = document.createElement("li");
      li.className = "goal-card";
      const total = g.shardPaths.length;
      li.innerHTML = `<p class="goal-card-title"></p><p class="goal-card-meta"></p>`;
      li.querySelector(".goal-card-title").textContent = g.title;
      li.querySelector(".goal-card-meta").textContent = `Завершён · ${total} / ${total} · ${themeLabel(g.themeId)}`;
      li.addEventListener("click", () => {
        state.currentCompletedId = g.id;
...
function refreshCompletedView() {
  const goal = state.completedGoals.find((x) => x.id === state.currentCompletedId);
  if (!goal) {
    state.view = "list";
    showView();
    return;
  }
  ensureCompletedFields(goal);
  ensureGoalShards(goal);

  document.getElementById("completed-title").textContent = goal.title;
  document.getElementById("completed-meta").textContent = `Завершён ${new Date(goal.completedAt).toLocaleDateString("ru-RU")} · ${themeLabel(goal.themeId)}`;

  const svg = document.getElementById("completed-svg");
  renderGlassSvg(svg, goal);
  const el = document.getElementById("completed-progress");
  if (el) el.textContent = "Витраж сохранён как завершённый";
}
...
function refreshGoalView() {
  ...
  const archiveBtn = document.getElementById("btn-archive-goal");
  if (archiveBtn) {
    const total = goal.shardPaths.length;
    const n = (goal.fills || []).length;
    archiveBtn.disabled = false;
    archiveBtn.textContent = n >= total ? "Сохранить как завершённый" : "Сохранить как завершённый (раньше)";
  }
...
function showView() {
  ...
  document.getElementById("view-completed").classList.toggle("hidden", state.view !== "completed");

  if (state.view === "list") refreshListView();
  if (state.view === "goal") refreshGoalView();
  if (state.view === "completed") refreshCompletedView();
  saveState(state);
}
...
function init() {
  ...
  state.completedGoals.forEach(ensureCompletedFields);
  ...
  document.getElementById("btn-completed-back").addEventListener("click", () => {
    state.currentCompletedId = null;
    state.view = "list";
    showView();
  });
  ...
  document.getElementById("btn-archive-goal").addEventListener("click", () => {
    const goal = state.goals.find((x) => x.id === state.currentGoalId);
    if (!goal) return;
    ensureGoalShards(goal);
    const total = goal.shardPaths.length;
    const n = (goal.fills || []).length;
    if (n < total) {
      if (!confirm("Витраж ещё не собран целиком. Всё равно сохранить как завершённый?")) return;
    }
    const archived = { ...goal, completedAt: Date.now() };
    ensureCompletedFields(archived);
    state.completedGoals.unshift(archived);
  ...
  document.getElementById("btn-delete-completed").addEventListener("click", () => {
    const goal = state.completedGoals.find((x) => x.id === state.currentCompletedId);
    if (!goal) return;
    if (!confirm("Удалить этот завершённый витраж из коллекции?")) return;
    state.completedGoals = state.completedGoals.filter((g) => g.id !== goal.id);
    state.currentCompletedId = null;
    state.view = "list";
    showView();
  });
